...
public class VJ01_SearchAndChooseTicketsOnASpecificDaySuccessfully extends BaseTest {
    ...
    public void TC01(Hashtable<String, String> data) {
        try {
            ...
            HomePage.getInstance().searchFlights(Boolean.parseBoolean(data.get("isReturn")),data.get("origin"), data.get("destination") , departDate,returnDate, data.get("currency"), Boolean.parseBoolean(data.get("isFindLowestPrice")), Integer.parseInt(data.get("adultsNum")));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
